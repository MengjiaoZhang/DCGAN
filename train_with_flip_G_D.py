...
1. Label Smoothing (training discriminator)
real sample: replace the label with a random number between 0.9 and 1.0,
fake samppl: replace it with 0.0 and 0.1.

2. Make the labels the noisy for the discriminator: 
occasionally flip the labels when training the discriminator

3. Flip labels when training generator: real = fake, fake = real

4. Maximize log(D(G(z))) instead of minimize 1-log(D(G(z)))

5. Sample z from a gaussian distribution

6. Each mini-batch needs to contain only all real images or all generated images and use batch norm

7. Avoid Sparse Gradients: ReLU, MaxPool:
LeakyReLU in both G and D
For Downsampling, use: Conv2d + stride
For Upsampling, use: ConvTranspose2d + stride

8. Use the ADAM Optimizer for both G and D
...
def train(netG, netD, optimizerD, optimizerG, dataloader, criterion, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        p = 'MNIST_DCGAN_results/Random_results/MNIST_DCGAN_with_flip_G_D_' + str(epoch + 1) + '.png'
        fixed_p = 'MNIST_DCGAN_results/Fixed_results/MNIST_DCGAN_with_flip_G_D_' + str(epoch + 1) + '.png'
